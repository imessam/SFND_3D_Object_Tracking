...
int main(int argc, const char *argv[])
{
    ...
    string yoloModelWeights = yoloBasePath + "yolov3.weights.big";
    ...
    std::string ttc_lidar[20]; // to vector and push
    std::string ttc_camera[20];
    ...
    for (string detectorType : detectors)
    {
        for (string descriptorType : descriptors)
        {
            ...
            for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex += imgStepWidth)
            {
                ...
                ttc_lidar[imgIndex + 1] += to_string(ttcLidar) + ",";
                ttc_camera[imgIndex + 1] += to_string(ttcCamera) + ",";

            } // eof loop over all images
            ttc_lidar[0] += detectorType + " & " + descriptorType + ",";
            ttc_camera[0] += detectorType + " & " + descriptorType + ",";
        }
    }

    map["ttc_lidar.csv"] = ttc_lidar;
    map["ttc_camera.csv"] = ttc_camera;

    for (const auto &it : map)
    {
        myfile.open(it.first);
        for (int i = 0; i <= imgEndIndex+1; i++)
        {
            myfile << it.second[i] << "\n";
        }
        myfile.close();
